...
br.getPageURI = function(index, reduce, rotate) {
	...
	if(level == 1)
	{
		...
		$.ajax({type: "POST", url: "../templates/bgconvert.php?level="+level+"&index="+index+"&imgurl="+book.imgurl+"&mode="+this.mode+"&issue="+book.issue, async: true , success :function(data){br.updater(data);} , data : {book:this.book.imglist}});
		return br.imagesBaseURL + "transparent.png";
	}
	else
	{
		$.ajax({type: "POST", url: "../templates/bgconvert.php?level="+level+"&index="+index+"&imgurl="+book.imgurl+"&mode="+this.mode+"&issue="+book.issue, async: true , data : {book:this.book.imglist}});
